...
| M2 4v4 调度、先制与确定性随机 | 多人队伍按实时速度行动，普通和先制顺序正确 | 未开始 |
| M3 技能、目标、MP 与数值效果 | 攻击和技能可合法选目标并结算 HP/MP | 未开始 |
| M4 状态、眩晕与速度变化 | 眩晕跳过行动，速度变化立即刷新顺序 | 未开始 |
...
剩余问题：无阻塞。敌人行动间歇时长暂为组件常量 AutoAdvanceDelaySeconds=0.6f；
  调度仍按单位创建顺序轮流，M2 由完整速度/先制规则替换
```

### [ ] M2 1 至 4 人多对多、先制与确定性调度
